...
document.getElementById("id").addEventListener("keyup", function() {
    if (/^[a-z]{1}[a-zA-Z0-9_-]{5,13}$/.test(this.value)) {
...
document.getElementById("pwd2").addEventListener("keyup", function() {
    ...
    if (pwd.value == this.value) {
        conf.innerText = "비밀번호 일치";
        conf.style.color = "green";
    } else if (this.value == "") {
        conf.innerText = null;
    } else {
        conf.innerText = "비밀번호 불일치";
...
document.getElementById("name").addEventListener("keyup", function() {
    const conf = document.getElementById("name-confirm");
    if (/^[가-힣]{2,5}$/.test(this.value)) {
        conf.innerText = "정상입력";
        conf.style.color = "green";
    } else {
        conf.innerText = "한글만 입력하세요";
        conf.style.color = "red";
    }
})
...
function validate(){
    ...
    const arr = document.querySelectorAll("input[type=\"radio\"]:checked");
    const arr2 = Array.from(arr, n => n.value);
    if (!arr2.length) {
        alert("성별을 선택해 주세요");
        return false;
    }
    ...
    */
    ...
    const va = document.getElementById("phone").value;
    if (va == "") {
        alert("전화번호를 입력해 주세요");
        return false;
    } else if (!/^0\d{1,2}-\d{3,4}-\d{4}$/.test(va)) {
